...
def plot_clusters(inflection_lengths, boundaries=None, save_path="clusters_plot.png"):
    ...
    if boundaries is not None:
        ...
        clusters = np.digitize(inflection_lengths, boundaries)
        cmap = plt.cm.Set1
    else:
    ...
    scatter = ax.scatter(
        inflection_lengths,
        np.zeros_like(inflection_lengths),
        c=clusters,
        cmap=cmap,
        s=100,
        edgecolors="k",
...
def main():
    ...
    parser.add_argument(
        "--cumulative_map_path",
        default="cumulative_map.json",
        help="JSON с lengths и cumulative_mAP",
    )
    parser.add_argument(
        "--thresholds_path",
        default="length_thresholds.json",
    ...
    parser.add_argument(
        ...
        default="cumulative_map_plot.png",
        help="Путь для сохранения графика кумулятивного mAP",
    )
    parser.add_argument(
        "--save_clusters_plot",
        default="clusters_plot.png",
        help="Путь для сохранения графика кластеров",
    )
    ...
    inflection_idx, inflection_lengths = find_inflection_points(
        lengths, cum_map, args.smooth_sigma
    )
    print(
        f"Found {len(inflection_lengths)} inflection points at lengths: {inflection_lengths}"
    )
    ...
    if args.thresholds_path:
        try:
            ...
            inflection_lengths = np.array(
                thr_data.get("inflection_points", inflection_lengths)
            )
    ...
    plot_cumulative_map(
        ...
        inflection_lengths,
        boundaries,
        save_path=args.save_cumulative_plot,
    )

    if len(inflection_lengths) > 0:
        plot_clusters(inflection_lengths, boundaries, save_path=args.save_clusters_plot)
